...
export default function YouTubeClipper({ onVideoLoad, onSaveClip }: YouTubeClipperProps) {
  ...
  const saveCurrentClip = async () => {
    ...
    setIsSaving(true);
    try {
      // TRANSCRIPTION DISABLED - SAVE BASIC CLIP INFO ONLY
      console.log('🔧 Transcription disabled - saving basic clip info only');

      // Create the clip object without transcription
      const newClip: SavedClip = {
        ...
        originalUrl,
        createdAt: new Date(),
        captions: [], // Empty captions array - no transcription
      };
      ...
      toast.success(`Clip "${newClip.title}" saved successfully! (No transcription)`);
